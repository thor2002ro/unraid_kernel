...
// 	ret = ioctl(bdev, SG_IO, (unsigned long)&sghdr);
// 	if (ret)
// 		return ret;
...
static int disk_hdparm(mdk_rdev_t *rdev, unsigned char ata_op)
{
	char *hddparm_arg;
	...
	snprintf( prefix, sizeof(prefix), "%s /dev/%s", hddparm_arg, rdev->name);
	...
	printk("arg0: %s", argv[0]);
	printk("arg1: %s", argv[1]);
	printk("hddparm_arg: %s", hddparm_arg);
	printk("dev: /dev/%s", rdev->name);
	*/

  return call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC);
}

static int disk_sg_start(mdk_rdev_t *rdev, unsigned char ata_op)
{
	char *hddparm_arg;
	//char userprog[] = "/usr/bin/logger";
