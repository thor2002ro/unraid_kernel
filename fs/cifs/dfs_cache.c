			    ...
			    struct dfs_info3_param **refs, int *numrefs)
{
	...
	*numrefs = 0;
	...
	if (!rc) {
		for (i = 0; i < *numrefs; i++)
			convert_delimiter(refs[i]->path_name, '\\');
	}
	return rc;
}
...
static void refresh_cache(struct cifs_ses **sessions)
{
	 ...
	 */
	down_read(&htable_rw_lock);
	for (i = 0; i < CACHE_HTABLE_SIZE && count < ARRAY_SIZE(ref_paths); i++) {
		struct hlist_head *l = &cache_htable[i];
		...
		hlist_for_each_entry(ce, l, hlist) {
			...
			ref_paths[count++] = kstrdup(ce->path, GFP_KERNEL);
		}
	}
	up_read(&htable_rw_lock);
	...
	for (i = 0; i < count; i++) {
		...
		if (unlikely(!ref_paths[i]))
			continue;

		xid = get_xid();
		rc = get_dfs_referral(xid, ses, ref_paths[i], &refs, &numrefs);
		free_xid(xid);

		if (!rc) {
			down_write(&htable_rw_lock);
			ce = lookup_cache_entry(ref_paths[i]);
			/*
